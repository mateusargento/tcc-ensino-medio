...
<h1><center>Tela do Administrador</center></h1>
...
<?php
	...
	echo "<h1 id='informacoes'>Compras:</h1>"; // Escreve o aviso

	$sql = mysql_query("SELECT * FROM compras_design"); // Verifica quantos regístros existem do usuário
	...
	{
		echo "<h1 id='informacoes'>Não existem compras de design ainda.</h1><hr>"; // Escreve o aviso
	}
	...
	while($row > 0) // Se tiver registro de compras do usuário
	{
		...
		$sql4 = "SELECT (email) FROM compras_design"; // Seleciona o email do comprador
		...
		while($busca = mysql_fetch_row($resultado) and $busca2 = mysql_fetch_row($resultado2) and $busca3 = mysql_fetch_row($resultado3) and $busca4 = mysql_fetch_row($resultado4)) // Enquanto houver resultados
		{
			...
			echo "<h1 id='informacoes'>Email do comprador: " . $busca4[0] . "</h1>"; // Escreve o email do comprador
	...
	$sql = mysql_query("SELECT * FROM compras_aplicativos"); // Verifica quantos regístros existem do usuário
	$row = mysql_num_rows($sql); // Conta quantos registros tem do usuário
	
	// -------------------------- Para pegar todas as compras de aplicativos --------------------------
	if($row == 0)
	{
		echo "<h1 id='informacoes'>Não existem compras de aplicativos ainda.</h1><hr>";
	}
	...
	while($row > 0)
	{
		...
		$sql4 = "SELECT (email) FROM compras_aplicativos";
		$resultado = mysql_query($sql);
		$resultado2 = mysql_query($sql2);
		$resultado3 = mysql_query($sql3);
		$resultado4 = mysql_query($sql4);
		while($busca = mysql_fetch_row($resultado) and $busca2 = mysql_fetch_row($resultado2) and $busca3 = mysql_fetch_row($resultado3) and $busca4 = mysql_fetch_row($resultado4))
		{
			...
			echo "<h1 id='informacoes'>Email do comprador: " . $busca4[0] . "</h1>";
			echo "<h1 id='informacoes'>Ticket de compra número #" . $busca3[0] . "</h1><hr>";
		}
		$row = $row - 1;	
	}
	
	$sql = mysql_query("SELECT * FROM compras_sistemas"); // Verifica quantos regístros existem do usuário
	$row = mysql_num_rows($sql); // Conta quantos registros tem do usuário
	
	// -------------------------- Para pegar todas as compras de sistemas --------------------------
	if($row == 0)
	{
		echo "<h1 id='informacoes'>Não existem compras de sistemas ainda.</h1><hr>";
	}
	...
	while($row > 0)
	{
		...
		$sql4 = "SELECT (email) FROM compras_sistemas";
		$resultado = mysql_query($sql);
		$resultado2 = mysql_query($sql2);
		$resultado3 = mysql_query($sql3);
		$resultado4 = mysql_query($sql4);
	...
	$sql = mysql_query("SELECT * FROM compras_sites"); // Verifica quantos regístros existem do usuário
	$row = mysql_num_rows($sql); // Conta quantos registros tem do usuário
	
	// -------------------------- Para pegar todas as compras de sites --------------------------
	if($row == 0)
	{
		echo "<h1 id='informacoes'>Não existem compras de sites ainda.</h1><hr> <br>";
	}
	...
	while($row > 0)
	{
		...
		$sql4 = "SELECT (email) FROM compras_sites";
		$resultado = mysql_query($sql);
		$resultado2 = mysql_query($sql2);
		$resultado3 = mysql_query($sql3);
		$resultado4 = mysql_query($sql4);
		while($busca = mysql_fetch_row($resultado) and $busca2 = mysql_fetch_row($resultado2) and $busca3 = mysql_fetch_row($resultado3) and $busca4 = mysql_fetch_row($resultado4))
		{
			...
			echo "<h1 id='informacoes'>Valor parcelado em: " . $busca2[0] . "</h1>";
			echo "<h1 id='informacoes'>Email do comprador: " . $busca4[0] . "</h1>";
			echo "<h1 id='informacoes'>Ticket de compra número #" . $busca3[0] . "</h1><hr><br>";
		}
		$row = $row - 1;	
	}
?>
...
<?php
	...
	error_reporting (E_ALL & ~ E_NOTICE & ~ E_DEPRECATED);
	mysql_connect('localhost','root','');
	mysql_select_db('esqueciasenha');
	
	echo "<h1 id='informacoes'>Solicitação de senha: </h1>"; // Escreve o aviso
	
	$sql = mysql_query("SELECT * FROM solicitacao WHERE estado = 'Nao resolvido'"); // Verifica quantos regístros existem do usuário
	$row = mysql_num_rows($sql); // Conta quantos registros tem do usuário
	
	// -------------------------- Para pegar todas as senhas esquecidas --------------------------
	if($row == 0)
	{
		echo "<h1 id='informacoes'>Não existem senhas esquecidas.</h1><hr> <br>";
	}
	
	while($row > 0)
	{
		$sql = "SELECT (email) FROM solicitacao";
		$sql2 = "SELECT (estado) FROM solicitacao";
		$sql3 = "SELECT (id) FROM solicitacao";
		$resultado = mysql_query($sql);
		$resultado2 = mysql_query($sql2);
		$resultado3 = mysql_query($sql3);
		while($busca = mysql_fetch_row($resultado) and $busca2 = mysql_fetch_row($resultado2) and $busca3 = mysql_fetch_row($resultado3))
		{
			echo "<h1 id='informacoes'>Email : " . $busca[0] . "</h1>";
			echo "<h1 id='informacoes'>Estado: " . $busca2[0] . "</h1>";
			echo "<h1 id='informacoes'>Ticket da solicitação número #" . $busca3[0] . "</h1><hr><br>";
		}
		$row = $row - 1;	
	}
...
<?php
	...
	echo "<h1 id='informacoes'>Perguntas: </h1>"; // Escreve o aviso

	$sql = mysql_query("SELECT * FROM perguntas WHERE solucao = ''"); // Verifica quantos regístros existem do usuário
	$row = mysql_num_rows($sql); // Conta quantos registros tem do usuário
	
	// -------------------------- Para pegar todas as perguntas --------------------------
	if($row == 0) // Se não tiver nenhum regístro de perguntas
	{
		echo "<h1 id='informacoes'>Não existem perguntas ainda.</h1><hr>"; // Escreve o aviso
	}
