...
<head>
	<meta charset="utf-8"/>
	<title> </title>
	<link rel="stylesheet" href="estilo_servicos.css"/>
...
<?php
	...
	mysql_select_db('servicos');

	$email = $_SESSION['login'];

	$sql = mysql_query("SELECT * FROM compras_design WHERE email_freelancer = '$email'"); // Verifica quantos regístros existem do usuário
	...
	{
		echo "<h1 id='informacoes'>Você não tem compras freelancer de design ainda.</h1><hr>"; // Escreve o aviso
	}
	
	while($row > 0) // Se tiver registro de compras do usuário
	{
		$sql = "SELECT (valtotal) FROM compras_design WHERE email_freelancer = '$email'"; // Seleciona o valor da compra
		$sql2 = "SELECT (parcelas) FROM compras_design WHERE email_freelancer = '$email'"; // Seleciona as parcelas da compra
		$sql3 = "SELECT (id) FROM compras_design WHERE email_freelancer = '$email'"; // Seleciona o id da compra
		...
		{
			echo "<h1 id='informacoes'>Compra freelancer do serviço de Design. R$: " . $busca[0] . ",00</h1>"; // Escreve o valor da compra
	...
	$sql = mysql_query("SELECT * FROM compras_aplicativos WHERE email_freelancer = '$email'"); // Verifica quantos regístros existem do usuário
	$row = mysql_num_rows($sql); // Conta quantos registros tem do usuário
	
	// -------------------------- Para pegar todas as compras de aplicativos --------------------------
	if($row == 0)
	{
		echo "<h1 id='informacoes'>Você não tem compras freelancer de aplicativos ainda.</h1><hr>";
	}
	
	while($row > 0)
	{
		$sql = "SELECT (valtotal) FROM compras_aplicativos WHERE email_freelancer = '$email'";
		$sql2 = "SELECT (parcelas) FROM compras_aplicativos WHERE email_freelancer = '$email'";
		$sql3 = "SELECT (id) FROM compras_aplicativos WHERE email_freelancer = '$email'";
		$resultado = mysql_query($sql);
		$resultado2 = mysql_query($sql2);
		$resultado3 = mysql_query($sql3);
		while($busca = mysql_fetch_row($resultado) and $busca2 = mysql_fetch_row($resultado2) and $busca3 = mysql_fetch_row($resultado3))
		{
			echo "<h1 id='informacoes'>Compra freelancer do serviço de Aplicativos. R$: " . $busca[0] . ",00</h1>";
	...
	$sql = mysql_query("SELECT * FROM compras_sistemas WHERE email_freelancer = '$email'"); // Verifica quantos regístros existem do usuário
	$row = mysql_num_rows($sql); // Conta quantos registros tem do usuário
	
	// -------------------------- Para pegar todas as compras de sistemas --------------------------
	if($row == 0)
	{
		echo "<h1 id='informacoes'>Você não tem compras freelancer de sistemas ainda.</h1><hr>";
	}
	
	while($row > 0)
	{
		$sql = "SELECT (valtotal) FROM compras_sistemas WHERE email_freelancer = '$email'";
		$sql2 = "SELECT (parcelas) FROM compras_sistemas WHERE email_freelancer = '$email'";
		$sql3 = "SELECT (id) FROM compras_sistemas WHERE email_freelancer = '$email'";
		$resultado = mysql_query($sql);
		$resultado2 = mysql_query($sql2);
		$resultado3 = mysql_query($sql3);
		while($busca = mysql_fetch_row($resultado) and $busca2 = mysql_fetch_row($resultado2) and $busca3 = mysql_fetch_row($resultado3))
		{
			echo "<h1 id='informacoes'>Compra freelancer do serviço de Sistemas. R$: " . $busca[0] . ",00</h1>";
			echo "<h1 id='informacoes'>Valor parcelado em: " . $busca2[0] . "</h1>";
			echo "<h1 id='informacoes'>Ticket de compra número #" . $busca3[0] . "</h1><hr>";
		}
		$row = $row - 1;	
	...
	$sql = mysql_query("SELECT * FROM compras_sites WHERE email_freelancer = '$email'"); // Verifica quantos regístros existem do usuário
	$row = mysql_num_rows($sql); // Conta quantos registros tem do usuário
	
	// -------------------------- Para pegar todas as compras de sites --------------------------
	if($row == 0)
	{
		echo "<h1 id='informacoes'>Você não tem compras freelancer de sites ainda.</h1><hr>";
	}
	
	while($row > 0)
	{
		$sql = "SELECT (valtotal) FROM compras_sites WHERE email_freelancer = '$email'";
		$sql2 = "SELECT (parcelas) FROM compras_sites WHERE email_freelancer = '$email'";
		$sql3 = "SELECT (id) FROM compras_sites WHERE email_freelancer = '$email'";
		$resultado = mysql_query($sql);
		$resultado2 = mysql_query($sql2);
		$resultado3 = mysql_query($sql3);
		while($busca = mysql_fetch_row($resultado) and $busca2 = mysql_fetch_row($resultado2) and $busca3 = mysql_fetch_row($resultado3))
		{
			echo "<h1 id='informacoes'>Compra freelancer do serviço de Sites. R$: " . $busca[0] . ",00</h1>";
			echo "<h1 id='informacoes'>Valor parcelado em: " . $busca2[0] . "</h1>";
			echo "<h1 id='informacoes'>Ticket de compra número #" . $busca3[0] . "</h1><hr>";
		}
		$row = $row - 1;	
